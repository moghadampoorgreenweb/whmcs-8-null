        ...
        <table class="datatable" id="relatedservicestbl" width="100%" border="0" cellspacing="1" cellpadding="3">
            ...
            {foreach $relatedservices as $relatedservice}
                <tr{if $relatedservice.selected} class="rowhighlight"{/if} data-original="true">
                    ...
                    <td>
                        {if   $relatedservice.regdate  eq  null}
                        {else}
                            {Morilog\Jalali\CalendarUtils::strftime('Y-m-d',$relatedservice.regdate)}
                        {/if}
                    </td>
                    <td>
                        {if   $relatedservice.nextduedate  eq  '-'}
                            {null}
                        {else}
                            {Morilog\Jalali\CalendarUtils::strftime('Y-m-d',$relatedservice.nextduedate)}
                        {/if}
                    </td>
